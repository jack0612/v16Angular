...
export class PolymorpheusOutletDirective<C extends object> implements OnChanges, DoCheck {
    ...
    @Input('polymorpheusOutlet') content: PolymorpheusContent<C> = '';

    @Input('polymorpheusOutletContext') context!: C;
    ...
    constructor(
        ...
        //input directive or templateRef
        //DI primitive templateRef
    private get template(): TemplateRef<unknown> {
        if (isDirective(this.content)) {//'ng-template[polymorpheus]'
    ...
    ngOnChanges({content}: SimpleChanges) {
        if (this.viewRef) {
            //TODO:
            (<any>this.viewRef).context = this.getContext();
        }
...
function isDirective<C extends object>(
    ...
    return content instanceof PolymorpheusTemplate;//'ng-template[polymorpheus]'
}
